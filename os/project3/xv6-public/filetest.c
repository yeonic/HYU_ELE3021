...
// #define NUM_BYTES2 (NTRPLINDIRECT * BSIZE)

#define NUM_TEST3 10

char buf[NUM_BYTES], buf2[NUM_BYTES];
// char buf1_2[NUM_BYTES2], buf2_2[NUM_BYTES2];
...
// void test1_2(int first)
// {
//   int fd;

//   if (first)
//     printf(1, "Test 1_2: Write %d bytes\n", NUM_BYTES2);
//   fd = open(filename, O_CREATE | O_WRONLY);
//   if (fd < 0)
//     failed("File open error\n");
//   if (write(fd, buf1_2, NUM_BYTES2) < 0)
//     failed("File write error\n");
//   if (close(fd) < 0)
//     failed("File close error\n");
//   if (first)
//     printf(1, "Test 1_2 passed\n\n");
// }
...
//   if (first)
//     printf(1, "Test 2_2: Read %d bytes\n", NUM_BYTES2);
//   fd = open(filename, O_RDONLY);
//   if (fd < 0)
//     failed("File open error\n");
//   if (read(fd, buf2_2, NUM_BYTES2) < 0)
//     failed("File read error\n");
//   for (i = 0; i < NUM_BYTES2; i++) {
//     if (buf2_2[i] != (i % 26) + 'a') {
//       printf(1, "%dth character, expected %c, found %c\n", i, (i % 26) + 'a', buf2[i]);
